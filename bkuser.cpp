...
void BKUser::setDefaultOptions() {
	...
	options.browserTextSize = 0; // Large
	options.browserDisplayMode = 2; // Smart Fit
	options.browserEnableFlash = false;
	options.browserInterfaceMode = 0; // Full
	options.browserConfirmExit = true;
	options.browserShowCursor = true;
}
...
void BKUser::save() {
	...
	fprintf(f, "\t\t<set option=\"browserTextSize\" value=\"%d\" />\n", options.browserTextSize);
	fprintf(f, "\t\t<set option=\"browserDisplayMode\" value=\"%d\" />\n", options.browserDisplayMode);
	fprintf(f, "\t\t<set option=\"browserEnableFlash\" value=\"%d\" />\n", options.browserEnableFlash ? 1 : 0);
	fprintf(f, "\t\t<set option=\"browserInterfaceMode\" value=\"%d\" />\n", options.browserInterfaceMode);
	fprintf(f, "\t\t<set option=\"browserConfirmExit\" value=\"%d\" />\n", options.browserConfirmExit ? 1 : 0);
	fprintf(f, "\t\t<set option=\"browserShowCursor\" value=\"%d\" />\n", options.browserShowCursor ? 1 : 0);
...
void BKUser::load() {
	...
	if (eoptions != 0) {
		...
		while (eset) {
			...
			else if (strncmp(option, "browserTextSize",      128) == 0) options.browserTextSize      = atoi(value);
			else if (strncmp(option, "browserDisplayMode",   128) == 0) options.browserDisplayMode   = atoi(value);
			else if (strncmp(option, "browserEnableFlash",   128) == 0) options.browserEnableFlash   = atoi(value) != 0;
			else if (strncmp(option, "browserInterfaceMode", 128) == 0) options.browserInterfaceMode = atoi(value);
			else if (strncmp(option, "browserConfirmExit",   128) == 0) options.browserConfirmExit   = atoi(value) != 0;
			else if (strncmp(option, "browserShowCursor",    128) == 0) options.browserShowCursor    = atoi(value) != 0;
	...
	if (options.browserTextSize < 0 && options.browserTextSize > 2) {
		options.browserTextSize = 0; // Large
		operror = true;
	}
	if (options.browserDisplayMode < 0 && options.browserDisplayMode > 2) {
		options.browserDisplayMode = 2; // Smart Fit
		operror = true;
	}
	if (options.browserInterfaceMode < 0 && options.browserInterfaceMode > 2) {
		options.browserInterfaceMode = 0; // Full
		operror = true;
	}
